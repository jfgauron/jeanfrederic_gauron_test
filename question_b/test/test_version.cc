...
TEST_CASE ("Semantic versioning 2.0.0") {
    version v = version("12.345.567-alpha.1.beta.2+exp.sha123");
    ...
    SECTION ( "version parse parts correctly" ) {
        ...
        REQUIRE(v.minor() == "345");
        REQUIRE(v.patch() == "567");
        REQUIRE(v.pre_release() == "alpha.1.beta.2");
    ...
    SECTION ( "is empty vs 1" ) {
        REQUIRE(version("") < version("1"));
    }

    SECTION ( "is 1 vs empty" ) {
        REQUIRE(version("1") > version(""));
    }

    SECTION (" is missing core version(s) ") {
        REQUIRE(version("1") < version("1.1"));
        REQUIRE(version("1.5") < version("1.5.1"));
        REQUIRE(version("1") == version("1.0"));
        REQUIRE(version("1.5") > version("1"));
    }
}